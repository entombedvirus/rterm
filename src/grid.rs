...
// Invariants
//  - max_rows >= num_rows
//  - each newline separated line in the rope will have len_chars be multiples of num_cols. This is
//  accomplished by padding lines with blank chars when necessary. This allows us to implement
//  soft-wrapping and be able to edit past the end of non-padded line.
impl Grid2 {
    ...
    pub fn num_current_display_rows(&self) -> usize {
        ...
        let rows = self.text.len_chars() as f32 / self.num_cols.0 as f32;
        rows.ceil() as usize
    }
    ...
    fn screen_range(&self) -> Range<usize> {
        let first = self.first_visible_line_no();
        first..(first + self.num_rows())
    }
    ...
    pub fn clear_screen(&mut self) {
        self.erase(
            (ScreenCoord(0), ScreenCoord(0)),
            (
                ScreenCoord(self.num_rows() - 1),
                ScreenCoord(self.num_cols() - 1),
            ),
        );
    }
    ...
    pub fn resize(&mut self, new_num_rows: usize, new_num_cols: usize) -> bool {
        ...
        if let Some(to_add) = new_num_rows.checked_sub(self.num_rows.0) {
            Self::append_n_blank_rows(&mut self.text, &new_blank_line, to_add);
        }

        // we should probably just pad short lines with blanks on the way out?
        //
        // if new_num_cols > self.num_cols {
        //     for line in self.text.lines() {
        //         if let Some(to_add) = new_num_cols.checked_sub(line.len_chars()) {
        //             let blank_str = &self.blank_line[..to_add];
        //             self.text.insert(0, blank_str);
        //         }
        //     }
        // }
    ...
    pub fn display_lines<'a, R: RangeBounds<usize>>(
        ...
        self.soft_wrapped_line_iter()
            .skip(query_range.start)
            .take(query_range.len())
            .map(|display_slice: ropey::RopeSlice<'a>| {
                let char_count = display_slice.len_chars();
                let num_blanks = self.num_cols() - char_count;
                let mut padded_text = String::with_capacity(self.num_cols());
                padded_text.extend(display_slice.chars());
                padded_text.push_str(&self.blank_line[..num_blanks]);
    ...
    pub fn write_text_at_cursor(&mut self, txt: &str) {
        ...
        let BufferCoord(char_idx) = self.screen_pos_to_char_idx(&self.cursor_state.position);
        // TODO: chars.count() -> graphemes.count()
        let mut txt_char_len = txt.chars().count();
        let pending_wrap = char_idx + txt_char_len == self.text.len_chars();
        self.text.remove(char_idx..txt_char_len);
        self.text.insert(char_idx, txt);
        if pending_wrap {
            txt_char_len -= 1;
            self.cursor_state.pending_wrap = true;
        }
        self.move_cursor_relative(0, txt_char_len as isize);
    }

    pub fn erase_from_cursor_to_eol(&mut self) {
        let (row, _) = self.cursor_state.position;
        self.erase(self.cursor_state.position, (row + 1, ScreenCoord(0)));
    }
    ...
    pub fn move_lines_down(&mut self, start_line_no: usize, n: usize) {
        ...
        let start_line_no = ScreenCoord(start_line_no);
        let char_idx = self.compute_screen_line_to_buf_char(start_line_no);
        Self::insert_n_blank_rows(&mut self.text, self.blank_line.as_ref(), char_idx, n);
    ...
    fn append_n_blank_rows(text: &mut ropey::Rope, blank_line: &str, num_rows: usize) {
        Self::insert_n_blank_rows(text, blank_line, BufferCoord(text.len_chars()), num_rows)
    }
    ...
    fn compute_screen_line_to_buf_char(&self, ScreenCoord(row): ScreenCoord) -> BufferCoord {
        let mut ret = 0;
        for line in self.soft_wrapped_line_iter().take(row) {
            ret += line.len_chars();
        }
        BufferCoord(ret)
    }

    fn remove_screen_rows(&mut self, rows: Range<ScreenCoord>) {
        let BufferCoord(start_idx) = self.compute_screen_line_to_buf_char(rows.start);
        let BufferCoord(end_idx) = self.compute_screen_line_to_buf_char(rows.end);
        self.text.remove(start_idx..end_idx)
    }

    fn erase(&mut self, from: (ScreenCoord, ScreenCoord), to: (ScreenCoord, ScreenCoord)) {
        let (from_row, from_col) = from;
        let (to_row, to_col) = to;
        assert!(to_row > from_row || (to_row == from_row && to_col >= from_col));
        ...
        self.text.insert(row_end, "\n");

        // middle complete rows
        self.remove_screen_rows(from.0 + 1..to.0);

        // ending partial row
        let BufferCoord(row_start) = self.screen_pos_to_char_idx(&(to.0, ScreenCoord(0)));
        let BufferCoord(row_end) = self.screen_pos_to_char_idx(&to);
        self.text.remove(row_start..row_end);
        self.text
            .insert(row_start, &self.blank_line[..row_end - row_start]);
        self.text.insert(row_end, "\n");
    ...
    fn screen_pos_to_char_idx(&self, (row, col): &(ScreenCoord, ScreenCoord)) -> BufferCoord {
        let row_idx = self.compute_screen_line_to_buf_char(*row);
        BufferCoord(row_idx.0 + col.0)
    }

    fn soft_wrapped_line_iter<'a>(&'a self) -> impl Iterator<Item = ropey::RopeSlice<'a>> + 'a {
        // +1 for newline
        let line_width = self.num_cols() + 1;
        assert!(self.text.len_chars() % line_width == 0);
        let offsets = 0..self.text.len_chars();
        offsets
            .step_by(line_width)
            .map(move |start| self.text.slice(start..start + line_width))
    }
}
...
mod tests {
    ...
    fn test_grid() {
        ...
        let display_lines = grid.display_lines(0..3);
        assert_eq!(grid.display_lines(0..3).count(), 3);
        assert_eq!(grid.text.len_chars(), 3 * (10 + 1));
    }
}
