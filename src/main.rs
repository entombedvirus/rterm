...
use egui::{CentralPanel, Color32, FontId, Key, Rect, ViewportBuilder};
use log::{error, info};
use nix::errno::Errno;
...
fn main() -> anyhow::Result<()> {
    ...
    let mut app = TerminalEmulator::new()
    ...
    eframe::run_simple_native("rterm", options, move |ctx, _frame| {
        ...
        if app.char_dimensions.is_none() {
            app.char_dimensions = Some(get_char_size(ctx, &app.font).into());
        }
...
struct TerminalEmulator {
    ...
    char_dimensions: Option<egui::Vec2>,
...
impl TerminalEmulator {
    ...
    fn update_win_size(&mut self, ui: &mut egui::Ui) -> anyhow::Result<()> {
        let rect = ui
            .ctx()
    ...
    fn paint_cursor(&self, ui: &mut egui::Ui) -> anyhow::Result<()> {
